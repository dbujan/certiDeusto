...
class LoginController {
    async loginAlumno(req, res) {
        ...
        const alumno = await database_1.default.promise().query('SELECT * FROM alumno WHERE email = ?', [email]);
    ...
    async loginProfesor(req, res) {
        const { email } = req.params;
        const profesor = await database_1.default.promise().query('SELECT * FROM profesor WHERE email = ?', [email]);
    ...
    async loginSecretaria(req, res) {
        const { email } = req.params;
        const secretaria = await database_1.default.promise().query('SELECT * FROM secretaria WHERE email = ?', [email]);
